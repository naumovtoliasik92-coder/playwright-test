...
test('Проверка атрибуда href, элементов навигации хедера', async ({ page }) => {
  ...
  await expect.soft(page.getByRole('link', { name: 'GitHub repository' })).toBeVisible('href', '"https://github.com/microsoft/playwright');
  await expect.soft(page.getByRole('link', { name: 'Discord server' })).toBeVisible('href', 'https://aka.ms/playwright/discord');
});
...
test('Проверка кнопки заголовка Get started', async ({ page }) => {
  await page.goto('https://playwright.dev/');
