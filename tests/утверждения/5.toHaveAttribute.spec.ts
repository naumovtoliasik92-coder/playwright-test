...
test('3. Проверка атрибутов изображения', async ({ page }) => {
  ...
  const alt1 = page.getByAltText('Аватар пользователя');
  await expect(alt1).toHaveAttribute('src', 'user.jpg');
  await expect(alt1).toHaveAttribute('alt','Аватар пользователя');
  await expect(alt1).toHaveAttribute('width', '200')
});
...
test('5. Проверка data-атрибутов', async ({ page }) => {
  ...
  const cont = page.getByText('Контейнер с data-атрибутами');
  await expect(cont).toHaveAttribute('data-role', 'container');
  await expect(cont).toHaveAttribute('data-visible', 'true');
  await expect(cont).toHaveAttribute('data-user-id', '12345');

  await page.locator('#update-data').click();

  await expect(cont).toHaveAttribute('data-visible', 'false');
  await expect(cont).not.toHaveAttribute('data-user-id', '12345');
