...
test.describe('Тестирование видимости элементов с toBeVisible()', () => {
  ...
  test('Базовый тест видимости элемента', async ({ page }) => {
    ...
    const visible = page.locator('#always-visible');
    await expect(visible).toBeVisible();
    await expect(visible).toHaveText('Всегда видимый элемент')
  });
  ...
  test('Тест изменения видимости элементов', async ({ page }) => {
    ...
    await expect(page.locator('#show-visibility')).toBeVisible();
    await expect(page.locator('#show-visibility')).toHaveCSS('visibility', 'visible');

    await page.locator('#show-opacity').click();
    await expect(page.locator('#show-opacity')).toBeVisible();
    await expect(page.locator('#show-opacity')).toHaveCSS('opacity', '1')
  });
